...
- [x] 创建 `frontend/src/utils/discussionStorage.ts` localStorage 管理
- [x] 创建 `frontend/src/components/DiscussionCartButton.tsx` 添加/移除按钮
- [x] 创建 `frontend/src/components/DiscussionBadge.tsx` 徽标组件
- [x] 实现讨论清单的增删改查
- [x] 实现最多 10 条限制和去重逻辑

#### 5.2 对话页面

- [x] 创建 `frontend/src/pages/Chat.tsx` 对话页面
  - 消息输入和发送
  - 对话历史展示
- [x] 实现与后端对话 API 的集成
- [x] 支持流式响应显示
