...
class Settings(BaseSettings):
    ...
    # AI Rate Limiting
    ai_rate_limit_per_minute: int = Field(default=10)   # gemini-flash: 10, gemini-flash-lite: 15
    ai_rate_limit_per_day: int = Field(default=250)     # gemini-flash: 250, gemini-flash-lite: 1000
    ai_enable_rate_limiting: bool = Field(default=True)
    ai_rate_limit_retry_delay: int = Field(default=60)  # 达到限制后等待秒数
    
    # Summary Generator
    summary_concurrency: int = Field(default=1)         # 摘要生成并发度, 免费模型速率限制, 只能串行了
