...
class CrawlService:
        ...
        async with AsyncSessionLocal() as db:
            ...
            for crawled_item in crawled_items:
                try:
                    ...
                    if item_exists:
                        logger.debug(
                            f"Item {crawled_item.external_id} already exists, skipping"
                        )
                        continue
    ...
    async def get_recent_items(
        self, source_id: Optional[str] = None, limit: int = 50
    ) -> List[Item]:
        """
        获取最近的条目
    ...
    async def get_crawl_stats(self) -> Dict[str, Any]:
        ...
        async with AsyncSessionLocal() as db:
            ...
            source_stats_stmt = select(
                Item.source_id, func.count(Item.id).label("count")
            ...
            recent_stmt = select(func.count(Item.id)).where(
                Item.fetched_at >= yesterday
            )
