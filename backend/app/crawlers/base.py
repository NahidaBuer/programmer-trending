...
class BaseCrawler(ABC):
    ...
    def validate_item(self, item: CrawledItem) -> bool:
        ...
        if not item.url or not item.url.startswith(('http://', 'https://')):
            logger.warning(f"Invalid item: invalid URL {item.url} for {item.external_id}")
            return False
    ...
    async def crawl_and_validate(self, limit: int = 30) -> List[CrawledItem]:
        ...
        try:
            ...
            logger.info(f"Crawled {len(items)} items from {self.source_name}, {len(valid_items)} valid")
